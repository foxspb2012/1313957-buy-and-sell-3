...
(() => {
  ...
  const updateOffersAfter = (offers) => {
    const offerNewestBlock = document.querySelector('.ticket-list__newest');
    const offerListWrapperElement = offerNewestBlock.querySelector('.tickets-list__wrapper');
    const offerListElements = offerListWrapperElement.querySelector('ul');
    offerListElements.innerHTML = ``;
    offers.forEach((offer) => {
      offerListElements.append(createOfferElement(offer));
    })
  };

  const updateOffersCommented = (offers) => {
    const offerDiscussedBlock = document.querySelector('.ticket-list__discussed');
    const offerListWrapperElement = offerDiscussedBlock.querySelector('.tickets-list__wrapper');
    const offerListElements = offerListWrapperElement.querySelector('ul');
    offerListElements.innerHTML = ``;
    offers.forEach((offer) => {
      offerListElements.append(createOfferElement(offer));
  ...
  socket.addEventListener('comment:delete', (offersCommented) => {
    updateOffersCommented(offersCommented);
  })
